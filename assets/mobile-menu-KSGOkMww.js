import{a as m}from"./vendor-Gv3iafvG.js";(function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))i(e);new MutationObserver(e=>{for(const s of e)if(s.type==="childList")for(const c of s.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&i(c)}).observe(document,{childList:!0,subtree:!0});function o(e){const s={};return e.integrity&&(s.integrity=e.integrity),e.referrerPolicy&&(s.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?s.credentials="include":e.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function i(e){if(e.ep)return;e.ep=!0;const s=o(e);fetch(e.href,s)}})();const v={toggleMenuDropdownBtn:document.querySelector(".js-dropdown-menu-desktop-item"),menuDropdown:document.querySelector(".js-menu-list-dropdown")};var g;(g=v.toggleMenuDropdownBtn)==null||g.addEventListener("click",S);function S(){v.menuDropdown.classList.toggle("is-open")}async function F(){const o="https://drinkify.b.goit.study/api/v1"+"/cocktails/",i={r:9};try{return(await m.get(o,{params:i})).data}catch(e){return console.error("Failed to fetch random cocktails:",e),null}}function N(t){const a="https://drinkify.b.goit.study/api/v1",o="/cocktails/search/",i=new URLSearchParams({f:t}),e=`${a}${o}?${i}`;return fetch(e).then(s=>{if(!s.ok)throw new Error(`HTTP error! status: ${s.status}`);return s.json()}).then(s=>s)}async function q(t){const i="https://drinkify.b.goit.study/api/v1"+"/cocktails/search/",e={s:t};return(await m.get(i,{params:e})).data}async function P(t){const i="https://drinkify.b.goit.study/api/v1"+"/cocktails/lookup/",e={id:t};return(await m.get(i,{params:e})).data[0]}async function y(t){const a="https://drinkify.b.goit.study/api/v1",o=`/ingredients/${t}`,i=a+o;return(await m.get(i)).data[0]}const d=new URL(""+new URL("sprite-CU8iVRKM.svg",import.meta.url).href,import.meta.url).href;function $({_id:t,drink:a,drinkThumb:o,description:i}){const c=(JSON.parse(localStorage.getItem("favoriteCocktails"))||[]).some(l=>l._id===t)?"active":"";return`<li class="cocktails-list-item list" data-testid="cocktail-card"  data-id="${t}">
      ...
      `).join("")}function I({_id:t,drink:a,drinkThumb:o,description:i,ingredients:e=[]}){const s=e.map(({title:c,measure:l,ingredientId:u})=>`<li class="modal-list-item js-modal-list-item" data-id='${u}'><a class='ingredients-btn link' href="#">${l?`${l} `:""}${c}</a></li>`).join("");return`<div class="modal-ingredients list" data-id="${t}">
  ...
  </div>`}const r={backdrop:document.querySelector(".backdrop")};function J(t){const a=JSON.parse(localStorage.getItem("favoriteCocktails"))||[];a.some(i=>i._id===t._id)||(a.push(t),localStorage.setItem("favoriteCocktails",JSON.stringify(a)))}function T(t,a){const o=a.querySelector('[data-type="addToFavorite"]');if(!o)return;let e=(JSON.parse(localStorage.getItem("favoriteCocktails"))||[]).some(c=>c._id===t._id);o.textContent=e?"remove from favorites":"add to favorite";const s=o.cloneNode(!0);o.replaceWith(s),s.addEventListener("click",()=>{let c=JSON.parse(localStorage.getItem("favoriteCocktails"))||[];e?(c=c.filter(l=>l._id!==t._id),s.textContent="add to favorite",e=!1):(c.push(t),s.textContent="remove from favorites",e=!0),localStorage.setItem("favoriteCocktails",JSON.stringify(c))})}r.backdrop.addEventListener("click",C);async function C(t){const o=t.target.closest(".js-modal-list-item").dataset.id;if(!o)return;const i=await y(o);document.querySelector(".modal-ingredients").classList.add("visually-hidden"),r.backdrop.innerHTML=w(i)}r.backdrop.addEventListener("click",B);function B(t){const a=t.target.closest(".js-ingredient-favorite-btn");if(!a)return;const o=a.dataset.id,i=JSON.parse(localStorage.getItem("favoriteIngredients"))||[];if(i.some(s=>s._id===o)){const s=i.filter(c=>c._id!==o);localStorage.setItem("favoriteIngredients",JSON.stringify(s)),a.textContent="add to favorite"}else y(o).then(s=>{i.push(s),localStorage.setItem("favoriteIngredients",JSON.stringify(i)),a.textContent="remove from favorite"})}//! Закриваємо вікно!!!
r.backdrop.addEventListener("click",function(t){const a=t.target.closest('[data-type="close-btn"]'),o=t.target===r.backdrop,i=t.target.closest('[data-type="backToCocktail"]'),e=t.target.closest('[data-type="backBtn"]'),s=JSON.parse(localStorage.getItem("currentCocktail"));if(a||o||e){r.backdrop.classList.add("visually-hidden"),r.backdrop.innerHTML="";return}i&&s&&(r.backdrop.innerHTML=I(s),r.backdrop.classList.remove("visually-hidden"),T(s,r.backdrop))});//! Закриваємо вікно!!!
...
//# sourceMappingURL=mobile-menu-KSGOkMww.js.map
